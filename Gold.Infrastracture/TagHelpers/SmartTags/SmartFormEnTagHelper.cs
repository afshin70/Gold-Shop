...
namespace Gold.Infrastracture.TagHelpers.SmartTags
{
    [HtmlTargetElement("smart-formEn")]
    public class SmartFormEnTagHelper : FormTagHelper
    {
        ...
        public string BackToLinkText { get; set; } = Captions.Cancel;

        [HtmlAttributeName(SubmitBtnTextAttributeName)]
        public string SubmitBtnText { get; set; } = Captions.SubmitInformation;
        ...
        public SmartFormEnTagHelper(IHtmlGenerator generator) : base(generator)
        {
        }
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            if (!IsPartialView)
            {
                ...
                {
                    searchBtn = $"<button type='button' id='submit-{FormId}' onclick='{SearchButtonJs};' class='btn btn-success btn-labeled'><span>{Captions.SearchEn}</span> <b><i class='fa fa-check-circle'></i></b></button>";
                    builder.AppendHtml($"{searchBtn}&nbsp;");
                }

                if (ExcelButton)
                {
                    excelBtn = $@"<button type = 'button' class='btn btn-info btn-labeled' onclick='javascript: GoToReport(""{ExcelUrl}"", ""#frm-search"")'><span>Excel Export</span><b><i class='fa fa-print'></i></b></button>";
                ...
                if (!RemoveButton)
                {
                    ...
                    cancelBtn = $"<button type='button' onclick=\"OpenWindowCreate('/{ControllerName}/{ActionName}',{parameters},'{ControllerName}');\" class='btn bg-slate btn-labeled legitRipple'><span>{Captions.CancelEn}</span> <b><i class='fa fa-times-circle'></i></b></button>";
