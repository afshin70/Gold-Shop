...
var PersianDatePicker = function () {
    ...
    function init() {
        ...
        _datePicker.setAttribute("id", "datePicker");
    ...
    this.show = function (textBox, today, elementType = 'input') {
        if (_datePicker == null) {
            init();
        }
        ...
        let datePickerTop = (elementType !== 'input' ? document.body.scrollTop : 0) + top + _textBox.offsetHeight;
        console.log("datePickerTop ", datePickerTop + 170)
        console.log("window.hight ", document.body.scrollHeight)
        if (datePickerTop + 170 > document.body.scrollHeight) {
            datePickerTop = datePickerTop - (170 + _textBox.offsetHeight);
        }

        _datePickerStyle.left = left + "px";
        _datePickerStyle.top = datePickerTop + "px";
    ...
    this.ModalShow = function (textBox, today) {
        if (_datePicker == null) {
            init();
        }
        ...
        _datePickerStyle.left = left + "px";
        _datePickerStyle.top = document.body.scrollTop + top + _textBox.offsetHeight + "px";
        // _datePickerStyle.top = top + _textBox.offsetHeight + "px";
        _datePickerStyle.visibility = "visible";
    ...
    this.showInModal = function (textBox, today, modalId) {
        if (_datePicker == null) {
            init();
        }
        ...
        _datePickerStyle.left = "50%";
        _datePickerStyle.top = window.scrollY + document.querySelector(modalId).querySelector(`#${_textBox.id}`).getBoundingClientRect().top + _textBox.offsetHeight + "px";
        _datePickerStyle.visibility = "visible";
    ...
    this.showPDatePicker = function (textBox, today,modalId='', elementType = 'input') {
      
        try {
            if (modalId.length>1) {
                this.showInModal(textBox, today, modalId);
            } else {
                this.show(textBox, today, elementType);
            }
        } catch (e) {
            console.log('error: ' + e);
            this.show(textBox, today, elementType);
...
PersianDatePicker.showPDatePicker = function (textBox, today,modalId='', type = 'input') {
    _persianDatePicker.showPDatePicker(textBox, today, modalId, type);
};
